...
**Design system (`app/components/`)**: a small, currently-unstyled component library (`Button`, `Link`, `TextInput`, `TextArea`, `Select`, `RadioGroup`, `Checkbox`, `Modal`, `Box`), modeled on the equivalent components in the `net-worth-tracker` project. Both folders and files use snake_case (e.g. `app/components/text_input/text_input.tsx`); only the exported component name is PascalCase (`TextInput`). No CSS yet — components exist for semantic/accessible structure (labels, `aria-describedby`, `aria-invalid`, etc.) and are meant to be skinned later without call sites changing. New forms/nav should be built from these rather than raw HTML elements. The target visual styling (colors, shadows, typography) isn't decided in this file — see the `zine-design-system` skill.

**Validation (Zod)**: route actions validate `FormData` with [Zod](https://zod.dev) schemas under `app/schemas/<model>.server.ts` — one file per model, a base `z.object()` plus named variants (`.extend()`/`.omit()`/`.pick()`), an exported flat union of that model's error codes, and an exported `is<Model>ErrorCode` type guard. Schema messages are semantic CODE strings (`"NAME_REQUIRED"`), never i18n keys and never English text — display copy is a route concern, not a schema concern, so each action builds its own local code→`t()` map, looks up each field's first issue in `result.error.issues` (never the deprecated `.flatten()`/`.format()`), and narrows it with the type guard (never a type assertion) before indexing into the map. There is no shared parse helper; actions call `schema.safeParse(Object.fromEntries(formData))` directly. The schema file is deliberately kept `.server.ts` (it pulls in `zod`, which a route's client bundle shouldn't need just for validation logic that only ever runs in an action) — a field constraint also needed client-side (e.g. a `maxLength` JSX attr) lives instead in a sibling, unsuffixed `<model>_constants.ts` that both the schema and the route's component import (see `app/schemas/item_constants.ts`). See the `zod-validation` skill for the full step-by-step recipe.

...
**Data layer**: `app/db.server.ts` builds a singleton `PrismaClient` over `@prisma/adapter-pg`, keyed via `app/singleton.server.ts` so HMR in dev doesn't spawn duplicate clients/connections. Route modules do not talk to Prisma directly — they go through functions in `app/models/*.server.ts` (`user.server.ts`, `magic_link.server.ts`, and eventually community/item/loan equivalents).

...
**Community IA**: community-scoped screens nest under `/communities/:communitySlug` as siblings — `items` (community-wide browse: a read-only list + detail pair, flat with no search/filter/category), `my_items` (the signed-in member's own items — list, create, view, edit, delete; see "`my_items` (owner-scoped item management)" below), `members`, `loans` — with `/communities` one level up as the only place "my communities" (join/leave/switch) is a page — it must not be nested inside a community's own section list as a peer of Items/My Items/Members/Loans. `Loan`/`Message` still aren't wired up yet. Items are modeled as fully separate listings per community (no shared item entity reused across communities). Switching communities navigates to a different community's route root, preserving the current leaf route where sensible (e.g. staying on `/loans`). **Privacy rule** (functional, not just visual — must hold in loaders/actions, not only be hidden client-side): the Items list and Item Detail never show who owns an item — every other member's item renders a neutral placeholder like "a neighbor" instead of the owner's name/avatar (a member's own item shows "You" instead, computed server-side so the raw ownership id is never sent to the client). A real name is only ever shown on My Loans, where the viewer is already a party to that specific loan. This doesn't apply to `my_items`, which only ever shows the viewer's own items. On the Members list, item ownership must never be shown or inferable at all (no per-type item counts, no click-through from a member to their listings) — only aggregate, non-identifying stats like total lend count are safe. Full visual design system (colors, shadows, typography, screen layouts, copy voice, accessibility floor) lives in the `zine-design-system` skill — read it before building any of these screens.

**`my_items` (owner-scoped item management)**: `app/routes/communities/$communitySlug/my_items/` is a nested route tree (`my_items.tsx` list, `new/new.tsx` create, `$itemId/` detail/edit/delete) for an item's owning member to manage it. It's strictly owner-only — unlike the `getCommunityMembership`/`meetsMinRole` role-check pattern below, the three item-scoped routes (`$itemId/$itemId.tsx`, `$itemId/edit/edit.tsx`, `$itemId/delete/delete.tsx`) each independently run one combined Prisma lookup, e.g.:
...
This collapses "item doesn't exist," "item belongs to a different community," and "item isn't yours" into one 404, with no `meetsMinRole` override for admins/owners — a moderation-override capability (letting an admin edit/delete another member's item) is a distinct, not-yet-built feature, deliberately not folded into this check. This mirrors `getCommunityMembership`'s own choice to collapse non-existence and non-membership into one signal rather than distinguishing them. The check is duplicated across the three routes rather than shared, per the "loaders/actions independently re-check what they need" convention below — an action can't consume a parent route's loader data anyway.

Deleting an item needing a confirmation step but no standalone page (`$itemId/delete/delete.tsx`) is a route module with only an `action` export (no `loader`, no default component) — the same resource-route shape `healthcheck.tsx` already uses with just a `loader`. It's invoked via a `<Form method="post">` inside a confirmation `Modal` (see Design system below), never navigated to directly.
...
**Design system (`app/components/`)**: a small, currently-unstyled component library (`Button`, `Link`, `TextInput`, `TextArea`, `Select`, `RadioGroup`, `Checkbox`, `Modal`, `Box`, `Table`), modeled on the equivalent components in the `net-worth-tracker` project. Both folders and files use snake_case (e.g. `app/components/text_input/text_input.tsx`); only the exported component name is PascalCase (`TextInput`). No CSS yet — components exist for semantic/accessible structure (labels, `aria-describedby`, `aria-invalid`, etc.) and are meant to be skinned later without call sites changing. New forms/nav should be built from these rather than raw HTML elements. The target visual styling (colors, shadows, typography) isn't decided in this file — see the `zine-design-system` skill.

**Validation (Zod)**: route actions validate `FormData` with [Zod](https://zod.dev) schemas under `app/schemas/<model>.ts` — one file per model, a base `z.object()` plus named variants (`.extend()`/`.omit()`/`.pick()`), an exported flat union of that model's error codes, and an exported `is<Model>ErrorCode` type guard. Schema messages are semantic CODE strings (`"NAME_REQUIRED"`), never i18n keys and never English text — display copy is a route concern, not a schema concern, so each action builds its own local code→`t()` map, looks up each field's first issue in `result.error.issues` (never the deprecated `.flatten()`/`.format()`), and narrows it with the type guard (never a type assertion) before indexing into the map. There is no shared parse helper; actions call `schema.safeParse(Object.fromEntries(formData))` directly. A field constraint also needed client-side (e.g. a `maxLength` JSX attr) is exported directly alongside the schema that uses it — see `NAME_MAX_LENGTH`/`DESCRIPTION_MAX_LENGTH` in `app/schemas/item.ts`. See the `zod-validation` skill for the full step-by-step recipe.

`Modal` (`app/components/modal/modal.tsx`) is the first design-system component built on a native interactive element (`<dialog>`) rather than a plain semantic wrapper. It's a controlled component (`isOpen`/`setIsOpen: (isOpen: boolean) => void`, plus `title`/`content`/`closeLabel`) rather than route- or state-driven — `isOpen` is synced to the dialog imperatively via `showModal()`/`close()` in a `useEffect`, since the declarative `open` HTML attribute alone only renders the dialog inline without the backdrop/focus-trap. A second effect listens for the dialog's native `close` event (fired on Esc) and calls `setIsOpen(false)`, so a native dismissal can't leave the caller's state out of sync with the dialog. `content` is fully caller-composed (no baked-in Cancel/Confirm semantics) — `Modal` only renders its own upper-right close button, wired to `setIsOpen(false)`.

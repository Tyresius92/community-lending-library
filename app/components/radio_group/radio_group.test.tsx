...
test("checks the option matching defaultValue", () => {
  render(<RadioGroup label="Visibility" name="visibility" options={options} defaultValue="private" />);
...
test("all options share the same name for form submission", () => {
  ...
  expect(screen.getByLabelText("Private")).toHaveAttribute("name", "visibility");
});

test("shows an error message", () => {
  render(<RadioGroup label="Visibility" name="visibility" options={options} errorMessage="Pick one" />);
  expect(screen.getByRole("alert")).toHaveTextContent("Pick one");
});

test("disables all options when disabled is set", () => {
  render(<RadioGroup label="Visibility" name="visibility" options={options} disabled />);
